...
LED LIGHTING CONTROL (SK9822/APA102, from led_test.py):
  The strip is driven directly from this script (same DotStar/SPI call as
  led_test.py) so LED brightness and color can be tuned in the SAME live
  session as exposure/gain/pipeline, against the same live detection
  feedback -- turbidity and low light both change how your own LED light
  scatters back into the lens (backscatter/glare), so the "best" camera
  settings and the "best" LED settings are not independent of each other.
  Live keys: i/k = LED brightness up/down, c = cycle color preset, l =
  toggle LEDs on/off. Starting state set via --led-brightness/--led-color/
  --led-off. --no-led skips LED setup entirely if the strip isn't wired
  up yet. Every trial row logs led_on/led_brightness/led_color alongside
  the existing camera/pipeline state.
"""
...
def setup_leds(num_pixels, brightness, baudrate):
    """SK9822/APA102 strip over hardware SPI -- same DotStar call as led_test.py."""
    return dotstar.DotStar(board.SCK, board.MOSI, num_pixels,
                            brightness=brightness, auto_write=False, baudrate=baudrate)


def apply_led_state(pixels, led_on, color, brightness):
...
def main():
    ...
    # --- LED lighting (SK9822/APA102, from led_test.py) ---
    led_group = parser.add_argument_group("LED lighting (SK9822/APA102)")
    ...
    led_group.add_argument("--led-brightness", type=float, default=0.3,
                            help="Starting LED brightness, 0.0-1.0 (default: 0.3). Start low "
                                 ...
                                 "fast as this climbs, especially at higher --turbidity-ntu.")
    led_group.add_argument("--led-baudrate", type=int, default=4000000,
                            help="SPI clock rate in Hz (default: 4000000). Lower this (e.g. "
                                 "400000) if an uncut/long strip or loose jumper wiring is "
    ...
    led_group.add_argument("--led-color", default="white",
                            ...
                            help="Starting LED color preset (default: white). Cycle live with 'c'.")
    led_group.add_argument("--led-off", action="store_true",
                            help="Start with LEDs off (e.g. an ambient-light-only baseline "
                                 "block). Toggle live with 'l'.")
    ...
    # before the capture loop starts) ---
    ...
    if led_enabled:
        pixels = setup_leds(args.led_num_pixels, led_brightness, args.led_baudrate)
        apply_led_state(pixels, led_on, LED_COLOR_PRESETS[led_color_idx][1], led_brightness)
        print(f"LEDs ready: {args.led_num_pixels} px, brightness={led_brightness:.2f}, "
    ...
    try:
        while True:
            ...
            if cmd == "q":
                ...
                apply_led_state(pixels, led_on, LED_COLOR_PRESETS[led_color_idx][1], led_brightness)
                print(f"\nLED brightness={led_brightness:.2f}")

            elif cmd == "c" and led_enabled:
                led_color_idx = (led_color_idx + 1) % len(LED_COLOR_PRESETS)
                apply_led_state(pixels, led_on, LED_COLOR_PRESETS[led_color_idx][1], led_brightness)
                print(f"\nLED color={LED_COLOR_PRESETS[led_color_idx][0]}")

            elif cmd == "l" and led_enabled:
                led_on = not led_on
                apply_led_state(pixels, led_on, LED_COLOR_PRESETS[led_color_idx][1], led_brightness)
                print(f"\nLED power: {'ON' if led_on else 'OFF'}")
