...
def main():
    ...
    pipeline_group.add_argument("--clahe", action=argparse.BooleanOptionalAction, default=True,
                                 help="Local contrast enhancement (default ON). --no-clahe to disable.")
    ...
    pipeline_group.add_argument("--denoise", action=argparse.BooleanOptionalAction, default=True,
                                 help="Bilateral denoise as the pipeline's last step (default ON).")
    ...
    print(f"Pipeline start state: dehaze={args.dehaze} white_balance={args.white_balance} "
          f"clahe={args.clahe}(clip={args.clahe_clip}) gamma={args.gamma} "
          f"denoise={args.denoise} underwater_tuning={args.underwater_tuning} "
    ...
    print("Controls: SPACE = log trial | n = force-log as NOT detected | "
          ...
          "1 = toggle dehaze | 2 = toggle white balance | 3 = toggle CLAHE | q = quit")
    ...
    try:
        while True:
            ...
            display = cv2.cvtColor(detect_input, cv2.COLOR_GRAY2RGB) if view_processed else frame.copy()
            ...
            pipe_label = (f"PIPE: dehaze={'ON' if dehaze_enabled else 'off'} "
                          ...
                          f"({args.clahe_clip:.1f}) gamma={args.gamma:.2f}")
            ...
            if cmd == "q":
                ...
                print(f"\nCLAHE: {'ON' if clahe_enabled else 'OFF'}")
