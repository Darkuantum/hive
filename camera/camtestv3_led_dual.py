...
LED LIGHTING CONTROL -- TWO STRIPS, SYNCED (SK9822/APA102, camtestv3_led.py
extended from one strip to two):
  Drives TWO independently-wired DotStar strips instead of one -- each has
  its own clock pin and data pin (--led1-clock-pin/--led1-data-pin and
  --led2-clock-pin/--led2-data-pin), since a Pi only exposes one hardware
  SPI clock/MOSI pair (board.SCK/board.MOSI) -- strip 2 defaults to a
  bitbang (software SPI) pin pair instead so both can run at once without
  fighting over the same hardware SPI bus. "In sync" here means every LED
  command (brightness, color, on/off) is applied to BOTH strips identically
  and in the same apply_led_state() call -- there's no way to drive them
  independently from this script; that's the point, they're meant to act
  as one wider/brighter light source. If one strip is unplugged this still
  runs fine as long as its object still constructs -- there is no per-strip
  hardware ACK, so a dark half is a wiring problem, not something the
  script can detect.
  Live keys: i/k = LED brightness up/down (both strips), c = cycle color
  preset (both strips), l = toggle LEDs on/off (both strips). Starting
  state set via --led-brightness/--led-color/--led-off. --no-led skips LED
  setup entirely if neither strip is wired up yet. Every trial row logs
  led_on/led_brightness/led_color alongside the existing camera/pipeline
  state (one set of values -- both strips always match).
"""
...
# while straight/cool white may carry further before red drops out. Worth
# testing explicitly whether color choice changes effective detection
# range, not just picking one and moving on.
LED_COLOR_PRESETS = [
    ("white", (255, 255, 255)),
...
CSV_FIELDS = [
    ...
    "filtered_x_m", "filtered_y_m", "filtered_z_m", "filter_state",
    "led_on", "led_brightness", "led_color",
    "auto_logged", "notes",
]
...
def apply_led_state(strips, led_on, color, brightness):
    """Applies the SAME state to every strip in `strips` -- this is what keeps
    the two strips in sync: there is no path in this script that updates one
    without the other."""
    for pixels in strips:
        pixels.brightness = brightness
        pixels.fill(color if led_on else (0, 0, 0))
        pixels.show()
...
def start_stdin_listener():
    ...
    def _reader():
        print("(headless mode) Type a command + Enter: space/s = log trial | n = force NOT-detected | "
              ...
              "3 = toggle underwater-tuning bundle (CLAHE+denoise+detector) | "
              "i/k = LED brightness up/down | c = cycle LED color | l = toggle LEDs | q = quit")
        while True:
            try:
                line = input()
            except EOFError:
            ...
            if cmd in ("", "space", "s"):
                q.put(" ")
            elif cmd in ("n", "e+", "e-", "g+", "g-", "a", "w", "1", "2", "3", "i", "k", "c", "l", "q"):
                q.put(cmd)
                if cmd == "q":
                    break
            else:
...
def main():
    ...
    # --- LED lighting (SK9822/APA102, two strips driven in sync) ---
    led_group = parser.add_argument_group("LED lighting (SK9822/APA102, two strips in sync)")
    led_group.add_argument("--led", action=argparse.BooleanOptionalAction, default=True,
                            help="Enable SK9822/APA102 LED strip control (default ON, both "
                                 "strips). --no-led to skip LED setup entirely -- e.g. if "
                                 "neither strip is wired up yet and you only want to tune "
                                 "camera/pipeline parameters for now.")
    ...
    led_group.add_argument("--led-brightness", type=float, default=0.3,
                            help="Starting LED brightness, 0.0-1.0 (default: 0.3), applied to "
                                 ...
                                 "especially at higher --turbidity-ntu.")
    ...
    led_group.add_argument("--led-off", action="store_true",
                            help="Start with both strips off (e.g. an ambient-light-only "
                                 "baseline block). Toggle live with 'l'.")
    ...
    # --- LED setup (independent of camera -- set up early so it's ready
    # before the capture loop starts). Two strips, each on its own pins,
    # always driven together -- see apply_led_state(). ---
    ...
    if led_enabled:
        ...
        apply_led_state(strips, led_on, LED_COLOR_PRESETS[led_color_idx][1], led_brightness)
        print(f"LEDs ready: 2 strips x {args.led_num_pixels}px "
              ...
              f"color={LED_COLOR_PRESETS[led_color_idx][0]}, on={led_on}")
    else:
        print("LEDs disabled (--no-led)")
    ...
    print("Controls: SPACE = log trial | n = force-log as NOT detected | "
          ...
          "1 = toggle dehaze | 2 = toggle white balance | 3 = toggle underwater-tuning bundle (CLAHE+denoise+detector) | "
          "i/k = LED brightness up/down | c = cycle LED color | l = toggle LEDs | q = quit")
    ...
    def log_trial(force_not_detected, is_auto=False):
        ...
        row = {
            ...
            "led_on": led_on if led_enabled else "",
            "led_brightness": round(led_brightness, 3) if led_enabled else "",
            "led_color": LED_COLOR_PRESETS[led_color_idx][0] if led_enabled else "",
            "auto_logged": is_auto,
    ...
    try:
        while True:
            ...
            led_label = (f"LED x2: {'ON ' if led_on else 'OFF'} "
                         f"{LED_COLOR_PRESETS[led_color_idx][0]} bri={led_brightness:.2f}") if led_enabled \
                else "LED x2: disabled (--no-led)"
            cv2.putText(display, led_label, (20, 345),
            ...
            if not args.no_preview:
                ...
                if time.time() - last_status_print >= 0.5:
                    ...
                    led_status = f"LED({'ON' if led_on else 'OFF'},{LED_COLOR_PRESETS[led_color_idx][0]}," \
                                 f"{led_brightness:.2f})" if led_enabled else "LED(disabled)"
                    print(f"\rlive: {status}  {meta_text}  [{ae_status}]  {led_status}   (trials logged: {trial_count})   ",
                          end="", flush=True)
            ...
            elif cmd in ("i", "k") and led_enabled:
                if cmd == "i":
