"""
camtestv5.py -- camtestv4.py, with the empirical x/y/z-correction multipliers
now DEFAULTED to values already calibrated for a specific target marker:
DICT_4X4_50, 50mm black-square side, ID 0 (x=1.1, y=1.15, z=2.0, in place of
v4's flat 1.6/1.6/1.6 starting point). Still live-tunable with the same
e/d, g/f, [/] keys if a re-check drifts.
...
USAGE:
  python3 camtestv5.py --condition-label "xyz_tune_100cm" \\
...
def main():
    ...
    window_name = "ArUco Test Harness v5 (id0 4x4_50 50mm, X/Y/Z tuning)"
