"""
camtestv5_100mm.py -- camtestv5.py, retargeted at a different physical
marker: DICT_4X4_50, 100mm black-square side, ID 0 (x=0.95, y=0.9, z=1.8,
in place of v5's id0/50mm 1.1/1.15/2.0 calibration). Still live-tunable
with the same e/d, g/f, [/] keys if a re-check drifts.

TARGET-ID LOCKING (new this version):
...
X/Y/Z TUNING (unchanged mechanism from camtestv4.py, new starting values):
  --x-correction / --y-correction / --z-correction default to 0.95 / 0.9 /
  1.8 -- calibrated for the id0 DICT_4X4_50 100mm marker this version
  targets, not v5's 50mm 1.1/1.15/2.0 values.
...
USAGE:
  python3 camtestv5_100mm.py --condition-label "xyz_tune_100cm" \\
...
def main():
    parser = argparse.ArgumentParser(description="ArUco test harness -- dehaze/CLAHE/LED/AE fixed "
                                                   "ON, X/Y/Z correction defaults calibrated for "
                                                   "id0 DICT_4X4_50 100mm marker")
    ...
    parser.add_argument("--marker-size", type=float, default=0.10,
                         help="Marker BLACK SQUARE side length in meters (default 0.10 = 100mm, "
                              "matching this script's calibrated target marker).")
    parser.add_argument("--x-correction", type=float, default=0.95,
                         help="Empirical multiplier on x_m only (default 0.95, calibrated for the "
                              "id0 DICT_4X4_50 100mm target marker). Live-tunable with e/d.")
    parser.add_argument("--y-correction", type=float, default=0.9,
                         help="Empirical multiplier on y_m only (default 0.9, calibrated for the "
                              "id0 DICT_4X4_50 100mm target marker). Live-tunable with g/f.")
    parser.add_argument("--z-correction", type=float, default=1.8,
                         help="Empirical multiplier on z_m only (default 1.8, calibrated for the "
                              "id0 DICT_4X4_50 100mm target marker). Live-tunable with [ / ].")
    ...
    window_name = "ArUco Test Harness v5 (id0 4x4_50 100mm, X/Y/Z tuning)"
