(() => {
  ...
  // from this app's manual/auto control_mode toggle above. Auto-leveling
  // (roll/pitch stabilization against tilt) only runs in STABILIZE --
  // MANUAL mode is raw stick passthrough with no correction at all,
  // regardless of what this app sends.
  ...
  async function pollState() {
    try {
      ...
      if (!userIsPickingFlightMode && m.mode) {
        const hasOption = Array.from(flightModeSelect.options).some((o) => o.value === m.mode);
        if (!hasOption) {
          const opt = document.createElement('option');
          opt.value = m.mode;
          opt.textContent = m.mode;
          flightModeSelect.appendChild(opt);
        }
        flightModeSelect.value = m.mode;
      }
