...
class MavlinkInterface:
    ...
    # ------------------------------------------------------------------
    def send_manual_control(self, x=0.0, y=0.0, r=0.0, z_neutral=True):
        """Send a MANUAL_CONTROL command -- this is ArduSub's actual
        intended companion-computer control path (per Blue Robotics:
        RC input isn't supported underwater at all, MANUAL_CONTROL over
        MAVLink is how a pilot's joystick normally talks to the vehicle).
        Use this instead of send_velocity() when running in STABILIZE
        (or MANUAL) rather than GUIDED -- it does not require a position
        estimate, which GUIDED does and which this platform has no
        source for (no GPS, no DVL).

        x, y, r: normalized -1.0 to 1.0 (surge, sway, yaw), same sign
        convention as your PID output -- but these are NOT m/s or rad/s,
        they're a fraction of "full stick deflection". Clamp your PID
        output to a sensible range before passing it in here.

        z_neutral: leave True -- there's no vertical thruster on this
        platform, so depth/throttle should always be neutral (500 in
        ArduSub's [0,1000] convention for this one axis specifically).
        """
        def to_1000(v):
            return int(max(-1000, min(1000, v * 1000)))

        mc_x = to_1000(x)
        mc_y = to_1000(y)
        mc_r = to_1000(r)
        mc_z = 500 if z_neutral else int(max(0, min(1000, 500)))
